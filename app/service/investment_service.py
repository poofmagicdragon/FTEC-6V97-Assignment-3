...
from app.service.user_service import get_logged_in_user, update_user_balance, get_user_by_username
...
# 
# service function: parameterized
def create_purchase_order(session: Session, username: str, portfolio_id: int, ticker: str, quantity_to_buy: int, balance: int
) -> str:
    ...
    if portfolio_id not in [p.id for p in get_all_portfolio_logged_in_user(session, username)]:
        _console.print(f"Portfolio ID {portfolio_id} does not exist.  Please enter a valid portfolio ID", style="red")
        return f"Portfolio ID {portfolio_id} does not exist.  Please enter a valid portfolio ID"    
    ...
    price = get_price_by_ticker(ticker, session)
    transaction = Transaction(user_id=username, portfolio_id=portfolio_id, security_id=ticker, trans_type="BUY", quantity=quantity_to_buy, price=price)
    session.add(transaction)

    new_balance = balance - (price * quantity_to_buy)
...
# wrapper
def create_purchase_order_console(session: Session, user: User) -> str:
    portfolio_id = int(_console.input("Portfolio ID: "))
    ticker = _console.input("Ticker: ").upper()
    quantity_to_buy = int(_console.input("Quantity: "))

    result = create_purchase_order(session, user, portfolio_id, ticker, quantity_to_buy)

    if "does not exist" in result or "Insufficient balance" in result:
        _console.print(result, style="bold red")
    else:
        _console.print(result, style="green")
...
def harvest_investment(session: Session, username: str, portfolio_id: int, ticker: str, quantity_to_sell: int, sell_price: int, balance: int) -> str:
    if portfolio_id not in [p.id for p in get_all_portfolio_logged_in_user(session, username)]:
        _console.print(f"Portfolio ID {portfolio_id} does not exist.  Please enter a valid portfolio ID", style="red")
        return f"Portfolio ID {portfolio_id} does not exist.  Please enter a valid portfolio ID"
    ...
    transaction = Transaction(user_id = username, portfolio_id = portfolio_id, security_id = ticker, trans_type = "SELL", quantity = quantity_to_sell, price = sell_price)
    session.add(transaction)

    new_balance = balance + (sell_price * quantity_to_sell)
    update_user_balance(session, username, new_balance)
